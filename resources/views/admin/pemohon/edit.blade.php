...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('admin.pemohon.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Nomor Induk Kependudukan</label>
                                <input type="number" class="form-control" name="nik" value="{{ $item->nik }}" required placeholder="Nomor Induk Kependudukan">
                                @if ($errors->has('nik'))
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="nama" value="{{ $item->name }}" required placeholder="Nama Lengkap">
                            </div>
                            <div class="form-group">
                                <label>Username</label>
                                <input type="text" class="form-control" name="username" value="{{ $item->username }}" required placeholder="Username">
                            ...
                            <div class="form-group">
                                <label>Email</label>
                                <input type="email" class="form-control" name="email" value="{{ $item->email }}" required placeholder="Email">
                                @if ($errors->has('email'))
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="tanggal_lahir" value="{{ $item->tanggal_lahir, date('Y-m-d') }}" required placeholder="Tanggal Lahir">
                            </div>
                             <div class="form-group">
                                <label>Nomor Handphone</label>
                                <input type="number" class="form-control" name="nomor_handphone" value="{{ $item->nomor_handphone }}" required placeholder="Nomor Handphone">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="provinsi" id="provinces" class="form-control" value="{{ old('provinsi') }}" required>
                                    ...
                                    @foreach (App\Models\Province::all() as $provincy)
                                        <option value="{{$provincy->id}}" {{ $provincy->name == $item->provinsi ? 'selected' : '' }}>{{ $provincy->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control" name="kota" value="{{ old('kota') }}" id="regencies">
                                    ...
                                    @foreach (App\Models\Regency::where('province_id', App\Models\Province::where('name', $item->provinsi)->first()->id)->get() as $kota)
                                    <option value="{{$kota->id}}" {{ $kota->name == $item->kota ? 'selected' : '' }}>{{ $kota->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control" name="kecamatan" value="{{ old('kecamatan') }}" id="districts">
                                    ...
                                    @foreach (App\Models\District::where('regency_id', App\Models\Regency::where('name', $item->kota)->first()->id)->get() as $kecamatan)
                                    <option value="{{$kecamatan->id}}" {{ $kecamatan->name == $item->kecamatan ? 'selected' : '' }}>{{ $kecamatan->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control" name="kelurahan" value="{{ old('kelurahan') }}" id="villages">
                                    ...
                                    @foreach (App\Models\Village::where('district_id', App\Models\District::where('name', $item->kecamatan)->first()->id)->get() as $kelurahan)
                                    <option value="{{$kelurahan->id}}" {{ $kelurahan->name == $item->kelurahan ? 'selected' : '' }}>{{ $kelurahan->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label>Alamat Sesuai KTP</label>
                                <textarea name="alamat" class="form-control"  required placeholder="Alamat Sesuai KTP">{{ $item->alamat }}</textarea>
                            </div>
